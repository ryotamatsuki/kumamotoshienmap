...
try {
  for (const spec of [{name:'desktop',w:1440,h:1000,mobile:false},{name:'mobile',w:390,h:844,mobile:true}]) {
    ...
    const needs=await page.evaluate(()=>{
      ...
      const controls=all('#needsView button,#needsView input,#needsView select,#needsView a').filter(visible).map(el=>({cls:String(el.className||''),h:el.getBoundingClientRect().height,text:(el.textContent||el.value||'').trim().slice(0,45)}));
    ...
    const volunteer=await page.evaluate(()=>{
      const n=s=>document.querySelector(s),all=s=>[...document.querySelectorAll(s)],visible=el=>{const s=getComputedStyle(el),r=el.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>0&&r.height>0};
      const h=s=>n(s)?.getBoundingClientRect().height??null,fs=s=>n(s)?parseFloat(getComputedStyle(n(s)).fontSize):null;
      const controls=all('#volunteerView button,#volunteerView input,#volunteerView select,#volunteerView a').filter(visible).map(el=>({cls:String(el.className||''),tag:el.tagName.toLowerCase(),h:el.getBoundingClientRect().height,text:(el.textContent||el.value||'').trim().replace(/\s+/g,' ').slice(0,55)}));
      return {open:fs('.vol-summary-open'),jump:h('.vol-summary-jump'),help:h('.vol-summary-help'),lowControls:controls.filter(x=>x.h<39.5).slice(0,40)};
    });
    ...
    const needLow=needs.lowControls.filter(x=>x.h<(spec.mobile?43.5:39.5));
    const volLow=volunteer.lowControls.filter(x=>x.h<(spec.mobile?43.5:39.5));
    ...
    if(volunteer.open<11||volunteer.jump<target||volunteer.help<target||volLow.length) throw new Error(`${spec.name}: Volunteer ${JSON.stringify(volunteer)}`);
    ...
    results.push({name:`dashboard-${spec.name}`,needs,volunteer,dashboard,map,overview,timeline,pageErrors,consoleErrors,needLowCount:needLow.length,volLowCount:volLow.length});
    await page.close();
