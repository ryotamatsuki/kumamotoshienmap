...
function usage() {
  console.log([
    ...
    "  --ledger=<path>                       UPDATE_LEDGER.jsonを検証",
...
function parseArgs(argv) {
  const options = {
    ...
    head: "HEAD",
    mode: null,
    requireLedgerIfDataChange: false,
  };
  const setMode = (mode) => {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === "--require-ledger-if-data-change") {
      options.requireLedgerIfDataChange = true;
      continue;
    }
    ...
    if (inlineMatch) {
      setValue(inlineMatch[1], inlineMatch[2]);
      continue;
    }
    const separateMatch = argument.match(/^--(ledger|base|head)$/u);
    if (separateMatch) {
      setValue(separateMatch[1], argv[++index]);
      continue;
    }
  ...
  if (options.mode === "staged" && (options.base || options.head !== "HEAD")) {
    fail("stagedモードでは--base/--headを併用できません。");
  }
...
function changedFiles(options) {
  ...
  if (options.mode === "staged") {
    output = runGit(["diff", "--cached", "--name-only"]);
  } else if (options.mode === "base") {
    output = runGit(["diff", "--name-only", `${options.base}...${options.head}`]);
  } else {
...
function validateNoUntrackedFiles() {
  ...
  if (files.length > 0) {
    fail(`未追跡ファイルがあります。先にstageしてexpected_changed_filesへ含めてください: ${files.join(", ")}`);
  }
}
...
function parseSingleLineObjectConstant(source, name) {
  ...
  try {
    ...
    fail(`${name}を解析できません: ${error.message}`);
  }
}
...
function assertSetEqual(expected, actual, label) {
  ...
  if (missing.length || unexpected.length) {
    fail(`${label}が不一致です。missing=${JSON.stringify(missing)} unexpected=${JSON.stringify(unexpected)}`);
  }
}
...
function validateLedger(ledger, ledgerPath) {
  ...
  if (ledger?.schema_version !== 1) fail(`${displayPath(ledgerPath)}のschema_versionは1である必要があります。`);
  ...
  if (!/^\d{8}-\d{4}$/u.test(String(ledger.release_id ?? ""))) {
    fail(`${displayPath(ledgerPath)}のrelease_idがYYYYMMDD-HHMM形式ではありません。`);
  }
  if (typeof ledger.reference_at !== "string" || Number.isNaN(Date.parse(ledger.reference_at))) {
    fail(`${displayPath(ledgerPath)}のreference_atがRFC3339日時ではありません。`);
  }
  if (ledger.page_checked_at !== null && (typeof ledger.page_checked_at !== "string" || Number.isNaN(Date.parse(ledger.page_checked_at)))) {
    fail(`${displayPath(ledgerPath)}のpage_checked_atが日時またはnullではありません。`);
  }
  if (ledger.gate_phase === "post_deploy" && ledger.page_checked_at === null) {
    fail(`${displayPath(ledgerPath)}のpost_deployではpage_checked_atが必須です。`);
  }
  if (!/^[0-9a-f]{40}$/iu.test(String(ledger.base_main_sha ?? ""))) {
    fail(`${displayPath(ledgerPath)}のbase_main_shaが40文字のSHA-1ではありません。`);
  }
  ...
  const normalizedExpected = ledger.expected_changed_files.map((file) => {
    if (typeof file !== "string" || !file.trim() || file.startsWith("/") || file.includes("..")) {
      fail("expected_changed_filesにはリポジトリ相対の安全なパスだけを指定してください。");
    }
  ...
  if (ledger.coverage === null || typeof ledger.coverage !== "object" || Array.isArray(ledger.coverage)) {
    fail("coverageはオブジェクトで必要です。");
  }
  if (!Array.isArray(ledger.unresolved)) fail("unresolvedは配列で必要です。");
  if (ledger.snapshots !== undefined && (ledger.snapshots === null || typeof ledger.snapshots !== "object" || Array.isArray(ledger.snapshots))) {
    fail("snapshotsはオブジェクトで必要です。");
  }
  if (ledger.unresolved.length > 0) fail(`UPDATE_LEDGER.unresolvedが空ではありません（${ledger.unresolved.length}件）。`);
  for (const [index, source] of ledger.sources.entries()) {
    if (!source || typeof source !== "object" || !source.source_id || !/^https?:\/\//u.test(String(source.url ?? ""))) {
      fail(`sources[${index}]にsource_idと有効なurlが必要です。`);
    }
  }
}
...
function validateParity() {
  ...
  if (!source.equals(readFileSync(DIST_HTML))) {
    fail("正本HTMLとdist/dashboard.htmlがバイト一致していません。");
  }
  for (const [sourcePath, distPath] of DIST_ASSETS) {
    ...
    if (!readFileSync(sourcePath).equals(readFileSync(distPath))) {
      fail(`${displayPath(sourcePath)}と${displayPath(distPath)}がバイト一致していません。`);
    }
  }
  ...
  for (const [label, value] of serverPairs) {
    if (!server.includes(JSON.stringify(value))) fail(`dist/server/index.jsに最新${label}が埋め込まれていません。`);
  }
  const index = readText(INDEX_HTML);
  if (!index.includes("./ehime_kumamoto_support_geocoded_shelters_20260802.html")) {
    fail("index.htmlが正本HTMLを参照していません。");
  }
}
...
function validateReleaseId(ledger) {
  ...
  const allIds = [
    ...extractReleaseIds(source, "正本HTML"),
    ...extractReleaseIds(published, "public/dashboard.html"),
  ];
  ...
  if (ledger && uniqueIds[0] !== ledger.release_id) {
    fail(`ledger.release_idとHTML queryが不一致です。expected=${ledger.release_id} actual=${uniqueIds[0]}`);
  }
  const validatorPaths = [
    resolve(ROOT, "scripts", "validate-current-state-audit.mjs"),
    resolve(ROOT, "scripts", "validate-built-site.mjs"),
  ];
...
function validateCurrentMetadata(ledger, releaseId) {
  ...
  let volunteerData;
  try {
    volunteerData = vm.runInNewContext(`(${volunteerMatch[1]})`, Object.create(null), { timeout: 1000 });
  } catch (error) {
    fail(`volunteer-data.jsのデータを解析できません: ${error.message}`);
  }
  const volunteerReleaseId = volunteerData?.meta?.release_id ?? volunteerData?.release_id;
  if (volunteerReleaseId !== undefined && volunteerReleaseId !== releaseId) {
    fail(`volunteer-data.jsのrelease_idとHTML queryが不一致です。expected=${releaseId} actual=${volunteerReleaseId}`);
  }
  ...
  if (Number.isNaN(Date.parse(pageCheckedAt)) || Number.isNaN(Date.parse(volunteerCheckedAt)) || Number.isNaN(Date.parse(referenceAt))) {
    fail(`現況metadataの日時が不正です。page=${pageCheckedAt} volunteer=${volunteerCheckedAt} reference=${referenceAt}`);
  }
  if (pageCheckedAt !== volunteerCheckedAt) {
    fail(`PAGE_RECHECK_META.checkedAtとVOLUNTEER_DATA.meta.checked_atが不一致です。page=${pageCheckedAt} volunteer=${volunteerCheckedAt}`);
  }
  if (pageMeta.volunteerCheckedAt && pageMeta.volunteerCheckedAt !== volunteerCheckedAt) {
    fail("PAGE_RECHECK_META.volunteerCheckedAtとボランティアデータのchecked_atが不一致です。");
  }
  if (ledger?.reference_at && referenceAt !== ledger.reference_at) {
    fail(`ledger.reference_atとVOLUNTEER_DATA.meta.reference_atが不一致です。expected=${ledger.reference_at} actual=${referenceAt}`);
  }
  if (ledger?.page_checked_at && pageCheckedAt !== ledger.page_checked_at) {
    fail(`ledger.page_checked_atとPAGE_RECHECK_META.checkedAtが不一致です。expected=${ledger.page_checked_at} actual=${pageCheckedAt}`);
  }
}
...
function validateCurrentShelterShape() {
  ...
  try {
    execFileSync(process.execPath, [validatorPath, `--current=${CURRENT_SHELTERS}`], {
      cwd: ROOT,
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
    });
  ...
  if (htmlMeta.currentCount !== data.meta?.current_count || htmlMeta.currentCount !== data.shelters.length) {
    fail(`current_countがHTML・meta・配列で一致しません。html=${htmlMeta.currentCount} meta=${data.meta?.current_count} rows=${data.shelters.length}`);
  }
...
function validateGitDiff(options) {
  const args = options.mode === "base"
    ? ["diff", "--check", `${options.base}...${options.head}`]
    : options.mode === "staged"
      ? ["diff", "--cached", "--check"]
      : ["diff", "--check"];
  runGit(args);
...
function validateBaseSha(ledger, options) {
  ...
  if (ledger.base_main_sha.toLowerCase() !== actualBase.toLowerCase()) {
    fail(`UPDATE_LEDGER.base_main_shaと--baseの解決値が不一致です。expected=${ledger.base_main_sha} actual=${actualBase}`);
  }
}
...
function validateLedgerCoverage(ledger, currentCount) {
  ...
  if (currentCoverage !== undefined && currentCoverage !== currentCount) {
    fail(`UPDATE_LEDGERのcurrent_countとcurrent-shelters.jsonが不一致です。expected=${currentCoverage} actual=${currentCount}`);
  }
  ...
  if (unresolved !== undefined && unresolved !== 0) fail(`UPDATE_LEDGERがunresolved施設を含みます: ${unresolved}`);
...
function validateDataChangeLedgerRequirement(ledger, files, required) {
  ...
  if (dataChanged && !ledger) {
    fail(`時点修正対象ファイルが変更されていますが、UPDATE_LEDGER.jsonがありません。files=${files.join(",")}`);
  }
}

const options = parseArgs(process.argv.slice(2));
const ledger = options.ledgerPath ? readJson(options.ledgerPath, "UPDATE_LEDGER.json") : null;
