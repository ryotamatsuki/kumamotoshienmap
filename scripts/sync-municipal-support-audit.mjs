...
function targetPageMeta(html) {
  ...
  const municipalRow = {
    ...
    current: "9月2日16:16に対口支援・他自治体支援を全件再監査",
...
function buildOverlay(pageMeta) {
  ...
  return [
    ...
    `SUPPORT_BLOCKS.forEach((block)=>{if(block.id!=='internal-coordination'){block.badge='旧割当履歴＋9月2日16:16個別再監査';block.note='割当名簿は履歴表示。現在の活動は派遣元公式情報を個別再監査し、CURRENT／PLANNED／HISTORICAL／UNKNOWNを分離。';}});`,
...
function patchStatic(html) {
  let next = html;
  next = next.replace("<div class=\"provider-card\"><b><i class=\"dot municipal\"></i>他自治体等</b><p>8月19日資料の対口支援・県内調整、モバイルファーマシー等。</p></div>","<div class=\"provider-card\"><b><i class=\"dot municipal\"></i>他自治体等</b><p>9月2日16:16に対口支援・他自治体支援を全件再監査。継続・終了・予定・UNKNOWNを分離。</p></div>");
  next = next.replace("<div class=\"section-note\">公表値：8月15日14時</div>","<div class=\"section-note\">旧割当履歴＋9月2日16:16個別再監査</div>");
  ...
  next = next.replace("<li>8月24日資料で愛媛県の人的支援を更新</li><li>8月19日資料：行政応援971人（関係機関含む計1,038人）</li><li>宇城市・八代市でモバイルファーマシー（8月8日資料の活動実績・最新状況要確認）</li>","<li>9月2日16:16に対口支援・他自治体支援を全件再監査</li><li>8月19日行政応援971人は履歴スナップショット（同一定義の基準日時点総数はUNKNOWN）</li><li>モバイルファーマシーは8月8日の活動履歴を保持し、8月24日の継続・終了は一次情報不足のためUNKNOWN</li>");
...
next=next.replaceAll("9月2日16:16",auditCheckedLabel).replaceAll("2026年8月24日18時06分再確認",auditCheckedSourceLabel);
