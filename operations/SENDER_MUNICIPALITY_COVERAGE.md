...
> Add-on to 時点修正更新オペレーション Version 2.5
>
> This document does not weaken Version 2.5. Existing shelter, municipal-support, national-support, Institution Coverage, Source Mention Closure and Release Gate checks remain mandatory.
...
- **Discovered sender closure**: every basic municipality or Tokyo special ward discovered in the required sources must be adjudicated.
...
`validate:release` first executes the existing Version 2.5 Release Gate and only then executes the sender coverage gate. Passing sender coverage can never bypass an existing release check.
